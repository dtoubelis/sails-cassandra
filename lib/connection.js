...
Connection.prototype.createTable = function(name, collection, cb) {
  // create able and indices
  cb();
};


Connection.prototype.dropTable = function(name, cb) {
  // drop table and associated indices
  this.cassandraClient.execute("DROP TABLE " + name + ";", function (err, result) {
    if (err) return cb(err);
    cb();
  });
};
