...
Collection.prototype.create = function(cb) {
  ...
  this.client.batch([ this.ddlHelper.cqlCreateTable ].concat(Object.values(this.ddlHelper.indexes)), function(err, result) {
...
Collection.prototype.drop = function(cb) {
  ...
  this.client.batch(cqlBatch, function(err, result) {
    if (err) return cb(err);
    cb();
  });
};
...
Collection.prototype.find = function(criteria, cb) {
 ...
 // validate cb
  ...
  if (!where || !_.isPlainObject(where)) {
    return cb(new Error("Invalid 'where' clause."));
  }

  // parse criteria
  var arr = this._parseCriteria(where);
  cql += " WHERE " + arr[0] + ";";
  cqlParams = arr[1];
  ...
  this.client.eachRow(cql, cqlParams, { prepare: true },
    function(n, row) {
      ...
      Object.keys(self.ddlHelper.colNamesToAttrNames).forEach(function(colName) {
        rs[self.ddlHelper.colNamesToAttrNames[colName]] = convertValueFromCassandraDriver(row[colName]);
      });
      results.push(rs);
    },
...
Collection.prototype.update = function(criteria, values, cb) {
  ...
  if (self.ddlHelper.pkAttrName in where) {

    // build update CQL
    cqlUpdateParams.push(where[self.ddlHelper.pkAttrName]);
    updates = [ _.cloneDeep(values) ];
    updates[0][self.ddlHelper.pkAttrName] = where[self.ddlHelper.pkAttrName];
    ...
    self.client.eachRow(cqlSelect, cqlSelectParams, { prepare: true },
      function(n, row) {
        ...
        upd[self.ddlHelper.pkAttrName] = convertValueFromCassandraDriver(row[pkColName]);
        updates.push(upd);
...
Collection.prototype.delete = function(criteria, cb) {
  ...
  if (self.ddlHelper.pkAttrName in where) {
    ...
    cqlDeleteParams = [ where[self.ddlHelper.pkAttrName] ];
 ...
 *
 */
function convertValueFromCassandraDriver(value) {
  var ret;
