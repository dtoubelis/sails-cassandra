...
const consistencies = require('cassandra-driver').types.consistencies;
...
Collection.prototype.describeTable = function(cb) {
  ...
  this.client.hosts.forEach(function(host) {
    if (!minVersion || semver.lt(semver.coerce(host.cassandraVersion), minVersion)) {
      minVersion = semver.coerce(host.cassandraVersion);
    }
  });
  ...
  if (semver.satisfies(minVersion, '2.x')) {
 ...
 * @api public
 */
Collection.prototype.query = function(queries, params, consistency, cb) {
  // validate cb
  if (!cb || !_.isFunction(cb)) {
    throw new Error("Callback is missing or not a function.");
  }
  ...
  if (_.isString(queries)) {
    // execute query
    if (typeof consistency == 'undefined') consistency = consistencies.one;
    this.client.execute(queries, params, { prepare: true, consistency: consistency }, function(err, result) {
      if (err) return cb(err);
      cb(null, result.rows);
    });
  }
